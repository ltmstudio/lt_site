  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      if (
        ...
        setIsDropdownOpen(false); // Закрываем dropdown
      }
    };
  ...
  return (
    <header className="h-[7.5%] shrink-0 flex justify-between items-center svgColor">
      <Link
        href="/"
        onClick={() => {
          handleLanguageChange(languages[0].code);
          onLogoClick(); // Сбрасываем активный пункт меню
        }}
      >
        <svg
          width="313"
      ...
      <div className="flex gap-5">
        <div className="relative flex items-center" ref={dropdownRef}>
          ...
          {isDropdownOpen && (
            <div className="absolute right-0 top-10 mt-2 w-40 bg-white rounded-lg shadow-lg z-10">
